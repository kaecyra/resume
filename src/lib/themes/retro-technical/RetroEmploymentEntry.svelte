...
<div class="mt-6">
  <div class="-mx-4 md:-mx-6 print:-mx-6 bg-[#1a2744] px-4 py-4 md:px-6 md:py-5 print:px-6 print:py-5 print:break-inside-avoid">
  ...
  {#if entry.description || entry.summary || entry.highlights.length > 0}
    <div class="px-4 pt-3 pb-2 md:px-5">
      {#if entry.description}
        <p class="mb-3 text-base italic leading-relaxed text-[#1a2744]/70 print:break-inside-avoid">
          {@html format_markdown(entry.description.trim())}
        </p>
      {/if}

      {#if entry.summary}
        <p class="mt-2 text-base leading-relaxed text-[#1a2744] print:break-inside-avoid">{@html format_markdown(entry.summary.trim())}</p>
      {/if}

      {#if entry.highlights.length > 0}
        <ul class="mt-3 space-y-1.5 border-t border-dashed border-[#c96620]/30 pt-3">
          {#each entry.highlights as highlight}
            <li class="flex gap-2 text-base leading-relaxed text-[#1a2744] print:break-inside-avoid">
              <span class="flex h-[1lh] shrink-0 items-center">
                <span class="inline-block h-1.5 w-1.5 rotate-45 bg-[#e87a2e]"></span>
              </span>
              <span>
                {#if highlight.title}
                  <span class="font-semibold uppercase text-[#c96620]" style="font-family: var(--retro-heading-font);">{highlight.title}:</span>
                {/if}
